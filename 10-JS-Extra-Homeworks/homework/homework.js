...
function numberOfCharacters(string) {
  ...
  for(letra in string){
    if(ocurrencias[string[letra]]){
      ocurrencias[string[letra]]+=1;
    } else{
      ocurrencias[string[letra]]=1;
    }
  }
...
function asAmirror(str) {
  ...
  for(i in cadenaPalabras){
    cadenaPalabrasInvertidas.push(cadenaPalabras[i].split('').reverse().join(''));
  }

  return cadenaPalabrasInvertidas.join(' ');
} 
...
function buscoInterseccion(arreglo1, arreglo2){
  ...
  return arreglo1.filter(function (numero) {
    return arreglo2.includes(numero);
